...
<script>
 $(function(){
	 $(".prj").on("click",'#deleteproject',function(){
		 if(confirm("삭제하시겠습니까?") == true){
			$("#frm").attr("method","post");
			$("#frm").attr("action","${cp}/project/deleteproject");
			$("#frm").submit();
			alert("프로젝트를 완전히 삭제합니다.")
		 }else{
			 return false;
		 }
	 });
	 $(".prj").on("click",'#restorepro',function(){
			$("#frm").attr("method","post");
			$("#frm").attr("action","${cp}/project/restorepro");
			$("#frm").submit();
			alert("프로젝트를 복구합니다.")
	 });
...
<%@ include file="/WEB-INF/views/common/left.jsp" %>
		...
		<section class="content">
			<header class="main-header clearfix">
				...
				<a href="/project/recyclebin" class="btn btn-light" ><i class="pe-7f-trash" style="font-size: 30px;"></i>
				</a>
			</header>
		...
		<c:forEach items="${prolist2}" var="pro">
			<div class="col-md-3 col-sm-6 prj" onclick="location.href='#'">
				<form action="/project/modifybtnpro" id="frm">
...
<%-- 		<c:if test="${pro.ctgry=='1'||pro.ctgry=='2'||pro.ctgry=='3' }"> --%>
				...
				<section class="condibox_1">
					...
					<button type="button" id="restorepro" class="btn btn-violet" >복구</button>
					<button type="button" class="btn btn-violet" id="deleteproject">삭제</button>
				<%-- 	<c:if test="${pro.notcwtatr=='1'}">						 
						<span class="fa fa-star" id="fa1" data-star="0" style="font-size: 40px; color : yellow; float:right;"></span>
					</c:if>
				
					<c:if test="${pro.notcwtatr=='0'}">						 
